######################################################################################
# Title: GEOAssist Agent V1.0 - An open-source autonomous research agent for geoscience data and literature. 
# Description: LLM driven natural language to generate summaries, bibliography lists, download PDF reports autonomously and extract and visualise domain data.
...
os.environ.setdefault("HF_TOKEN", "Your Free Hugging Face KeyHere") # You can of course down load an open model like Gemma, Llama etc locally
...
DDG_AGENT_ROUNDS_DEFAULT = int(os.getenv("DDG_AGENT_ROUNDS_DEFAULT", "60"))
...
def _downloads_dir() -> str:
    """
    Return a writable folder for saved PDFs: <home>/Downloads/Geological_AI_download
    (or DOWNLOAD_DIR/Geological_AI_download if DOWNLOAD_DIR is set).
    Falls back to home or CWD if ~/Downloads doesn't exist.
    """
    # Allow an override base via env
    base = os.getenv("DOWNLOAD_DIR")
    if not base:
        home = os.path.expanduser("~")
        candidates = [
            os.path.join(home, "Downloads"),  # typical on Windows/macOS/Linux
            home,
            os.getcwd(),
        ]
        for c in candidates:
...
def run_simple(
    ...
    """
    Returns (answer_plus_sources_md, gif_path_or_None, macrostrat_md, macrostrat_rows)
    """
    question = (question or "").strip()
    if not question:
        return ("Please enter a research question.", None, "Enter a query to fetch Macrostrat results.", [], "### DuckDuckGo PDF Agent — Summary\n\n_Agent disabled._")
    # --- References (quiet)
    ref_target = int(max(5, min(ref_target, REF_MAX)))
    refs = discover_references(question, ref_target=ref_target)

    # --- Sources index & short answer (~50 words)
    ...
    sources_md = format_sources_md(refs)  # single numbered list only
    final_md = f"### LLM Generated Snippet\n\n{answer}\n\n## Sources\n\n{sources_md}"

    # --- Optional GPlates (quiet)
    gif_path: Optional[str] = None
    if enable_gplates:
        # LLM-first age range (fallback to regex/ICS)
        ...
        if start_ma < end_ma:
            start_ma, end_ma = end_ma, start_ma  # ensure older->younger
        ...
        if bbox:
            bbox = expand_bbox(
                bbox,
                factor=BBOX_EXPAND,
                min_lon_span=BBOX_MIN_LON_DEG,
                min_lat_span=BBOX_MIN_LAT_DEG,
                pad_deg=BBOX_PAD_DEG,
            )
        else:
            # Roomy local fallback around the inferred point instead of going global
        ...
        gif = build_gplates_animation_gif_fast(
            ...
            max_frames=12,
            model=gplates_model,
            extent=bbox,                     # pass the LLM-derived extent
        ...
        if gif and os.path.isfile(gif) and os.path.getsize(gif) > 0:
            ...
            try:
                gif = create_local_placeholder_gif(question)
            except Exception:
                gif = None
            if gif and os.path.isfile(gif) and os.path.getsize(gif) > 0:
                gif_path = gif

    # --- Macrostrat (always try; very fast)
    macro_md, macro_rows = query_macrostrat_from_text(question)

# --- DuckDuckGo PDF Agent (optional) ---
    ddg_summary_md = "### DuckDuckGo PDF Agent — Summary\n\n_Agent disabled._"
    year_hist_path = None
    if enable_ddg_agent:
        ...
        # NEW: make the histogram
        year_hist_path = _save_pdf_year_histogram(details)    
        # Brief note in the main “Answer + Sources” panel
        final_md += "\n\n## PDF Agent\n"
        final_md += f"- Saved **{len(saved)}** PDF(s) to `{_downloads_dir()}`.\n"
        for p in saved:
            final_md += f"  - {os.path.basename(p)}\n"
    
        # Detailed summary with queries, domains, and per-PDF 250-word snippets
        L = ["### DuckDuckGo PDF Agent — Summary", ""]
        if queries_used:
            L.append("**Queries used:**")
            seen_q = set()
            for qx in queries_used:
                if qx in seen_q:
                    continue
                seen_q.add(qx)
                L.append(f"- `{qx}`")
        ...
        if details:
            ...
            for d in details:
                ...
                L.append("_No PDFs downloaded._")

   
        ddg_summary_md = "\n".join(L)
    
        return (final_md, gif_path, macro_md, macro_rows, year_hist_path, ddg_summary_md)
...
with gr.Blocks(title="Geological Deep Research Agent for Literature and Data Searches") as demo:    
        ...
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:10px;">
          ...
          <div>
            ...
            <p style="margin:8px 0 0 0; font-size:clamp(16px, 1.4vw, 22px); line-height:1.6;">
              An open-source autonomous research agent for geoscience data and literature. LLM driven natural language to generate summaries, bibliography lists, download PDF reports autonomously and extract and visualise domain data.
            </p>
          </div>
        </div>
    """)
    ...
    go.click(
        fn=run_simple,
        inputs=[q, refs, do_gplates, anim_ms, gplates_model, do_ddg, ddg_rounds, ddg_max_per],
        outputs=[out, gplates_img, macro_md, macro_tbl, year_hist_img, ddg_agent_md],
    )
    # MAIN AND LAUNCH #####################################################################################
